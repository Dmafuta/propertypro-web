...
const SuperAdmin2FAForm = () => {
  ...
  useEffect(() => {
    ...
    // OTP already sent during login — start 30 s cooldown before resend
  ...
  const handleResend = async () => {
    ...
    startTimer(30, () => setResendDisabled(false));
    try {
      await axiosInstance.post('/auth/superadmin/resend-2fa', { tempToken });
    } catch (e: any) {
  ...
  const handleVerify = async () => {
    ...
    try {
      const data: any = await axiosInstance.post('/auth/superadmin/verify-2fa', {
        tempToken,
        code: otp,
      });
      ...
      sessionStorage.removeItem('2fa_tenant_slug');
  ...
  return (
    <Stack sx={{ height: 1, alignItems: 'center', justifyContent: 'space-between', pt: { md: 10 }, pb: 10 }}>
      <Box sx={{ display: { xs: 'none', md: 'block' } }} />

      <Grid container sx={{ maxWidth: '35rem', rowGap: 4, p: { xs: 3, sm: 5 }, mb: 5 }}>
        <Grid size={12}>
          ...
          >
            <Grid container>
              ...
              <Grid size={12} sx={{ textAlign: 'center' }}>
                <Link href="/superadmin/login" variant="subtitle2">
                  Back to login
                </Link>
              </Grid>
            </Grid>
...
export default SuperAdmin2FAForm;
